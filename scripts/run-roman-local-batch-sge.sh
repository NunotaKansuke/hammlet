...
# SGE copies submitted scripts into its spool directory.  Keep the repository
# and interpreter absolute so this wrapper remains valid after that copy.
...
exec "${python_bin}" "${repo_dir}/scripts/run-roman-local-batch.py" "$@"
