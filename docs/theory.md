...
`(map, alpha, observation)` to compact Fourier moments over
`(map, radial-node, mode)`. The FFT itself is only part of the speedup; the
...
At every `(map, geometry, alpha)`, Hammlet profiles the two linear fluxes
...
`(M_f,N_{alpha,f})` only on a union of:
